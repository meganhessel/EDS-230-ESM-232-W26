...
-   what issue jumps out - what might be the cause?
    -   ISSUE: 1944 measurements are super small!
    -   BECAUSE: only started tracking halfway in 1944.
-   how would you fix this?
    -   FIX: remove 1944 entirely
...
```{r}
# Swicthed the function to eff (parameter) of 0.6 
site2 <- solarpv(area = 0.1, 
          solar = sierraczosolar, clr = "green", eunit = "kW", eff = 0.6, g = TRUE)


# Find percent change (new - base / base)
percent_change <- ((site2$mean -  site1$mean) / site1$mean) * 100
```
