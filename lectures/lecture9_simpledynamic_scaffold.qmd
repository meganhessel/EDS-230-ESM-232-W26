...
Ptime <- years %>% 
  map_dbl(~exppop(P0=20, r=0.01, T=.x)) #run model for each yeah 

# keep track of what times we ran by adding years to data.frame
Ptime <-data.frame(Ptime, years)

# plot
ggplot(Ptime) + 
  geom_point(aes(x = years, y = Ptime)) + 
  labs(y = "Rabbits")
```
...
dexppop # Adding d = differential equation - returns dP/dt
