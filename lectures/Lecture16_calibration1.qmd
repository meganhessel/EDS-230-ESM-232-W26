...
res = msage %>% 
  ...
  map_dbl(nse, o=msage$obs) # map applies function to lots of data, return double 
...
# subset best parameter set 
ggplot(msage, aes(as.Date(date), msage[,best_par]))+
  geom_line(col="red")+ # observed 
  geom_line(data=msage, # Best parameter set 
            aes(as.Date(date), obs), col="black", alpha = 0.7)+
...
```{r}
# Compute metrics for Relative Error 
compute_RE = function(m, o) {
  err = m - o
  meanobs = mean(o)
  return(mean(err) / meanobs)
} 

# Run metrics over dataset 
res <- msage %>% 
  select(-date, -month, -day, -year, -wy, -obs ) %>% 
  map_dbl(compute_RE, o=msage$obs)
...
res <- msage %>% 
  select(-date, -month, -day, -year, -wy, -obs ) %>% 
  map_dbl(compute_mas, o=msage$obs)
...
```{r}
# Compute metrics for Relative Error 
compute_rmse = function(m, o) {
  err = (m - o)^2
  n = length(o)
  return(sqrt(sum(err) /n))
} 

# Run metrics over dataset 
res <- msage %>% 
  select(-date, -month, -day, -year, -wy, -obs ) %>% 
  map_dbl(compute_rmse, o=msage$obs)
