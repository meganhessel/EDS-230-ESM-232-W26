...
ORDINARY DIFFERINTAL EQUATIONS = ODE
...
X1 <- cbind.data.frame(r = r, K = K) # put together 
...
X1 <- X1 %>% map_df(pmax, 0.0) # set the growth rates to 0 
X2 <- X2 %>% map_df(pmax, 0.0)
# ^^^ for every max in the dataframe, set it to 0 

# create our SOBOL OBJECT and get sets ofparameters for running the model
...
compute_pop_metrics <- function(result, thresh) {
  ...
  # ^^ first time the population is greater than the threshold 
  
  # account for sitation with threshold is never reached
  idx <- ifelse(is.na(idx), length(result$P), idx)
  threshyear <- result$time[idx] # return actual value 
...
# Test that it works- SIMULATION TIMES 
...
# turn ode results into a labelled data frame - 1st col & 2nd col
...
Do what just did, and now make it a wrapper function with all the parameters. :)

wrapper funtion - RUN THE ODE AND EXTRACT RESULTS for all parameters at once
...
USE THE WRAPPER FUNCTION FOR EVERY ROW OF parameter sets (that sobol object created) with pmap
...
allresults <- as.data.frame(sens_P$X) %>% 
  pmap(p_wrapper, 
       Pinitial = Pinitial, simtimes = simtimes,
       odefunc = dpopgrowth, metricfunc = compute_pop_metrics,
       metricpars=list(thresh=100))
...
# ^^ go caculate the senstivity indicies 
